...
function extractRetardUnit(str) {
  if (isRangeSelf(str)) {
    return { amount: -1, type: Range.SELF };
  }
  if (isRangeTouch(str)) {
    return { amount: 0, type: Range.TOUCH };
  }
  if (isRangeSpecial(str)) {
    return { amount: -1, type: Range.SPECIAL };
  }
  if (isRangeSight(str)) {
    return { amount: -1, type: Range.SIGHT };
  }
  if (isRangeUnlimited(str)) {
    return { amount: -1, type: Range.UNLIMITED };
  }
  ...
  switch (unit) {
    case "feet":
      return { amount, type: Range.DISTANT, unit: "feet" };
    case "mile":
      return { amount, type: Range.DISTANT, unit: "mile" };
    default:
...
function convertRetardUnitToMetric(str) {
  ...
  switch (type) {
    ...
    case "distant":
      switch (unit) {
        case "feet":
          return { amount: feetToMeters(amount), type, unit: "meter" };

        case "mile":
          return { amount: mileToKilometers(amount), type, unit: "kilometer" };
        default:
...
function translateDurationUnit(lang, isUpTo, amount, durationUnit) {
  ...
  if (!unitTranslation) {
    throw new Error(`translateDurationUnit: missing for ${lang} : ${durationUnit}`)
  }
  if (isUpTo) {
...
function translateCastingTimeUnit(lang, castingTimeUnit) {
  ...
  if (!translated) {
    throw new Error(`translateCastingTimeUnit: missing for ${lang} : ${castingTimeUnit}`)
  }
  return translated
}
...
function translateMagicSchool(lang, magicSchool) {
  ...
  if (!translated) {
    throw new Error(`translateMagicSchool: missing for ${lang} : ${magicSchool}`)
  }
  return translated;
}
